        ...
        foreach (var trackedImg in image.updated)
            ...
            if (trackedImg.trackingState == TrackingState.Tracking)
            {
                if (trackedImg.referenceImage.guid == s_FirstImg)
                {
                    ...
                    firstPrefab.transform.SetPositionAndRotation(trackedImg.transform.position,
                        trackedImg.transform.rotation);
                }
                else if (trackedImg.referenceImage.guid == s_SecondImg)
                {
                    secondPrefab.SetActive(true);
                    secondPrefab.transform.SetPositionAndRotation(trackedImg.transform.position,
                        trackedImg.transform.rotation);
                }
            }
            ...
            else
            {
                ...
                else if (trackedImg.referenceImage.guid == s_SecondImg) secondPrefab.SetActive(false);
            }

        foreach (var trackedImg in image.removed)
            if (trackedImg.referenceImage.guid == s_FirstImg)
                Destroy(firstPrefab);
            else if (trackedImg.referenceImage.guid == s_SecondImg) Destroy(secondPrefab);
    }
}